...
- **Plans / Pricing**: 3 tiers — Starter (₹49 launch / ₹99 reg, 10 posts), Growth (₹99 / ₹199, 15 posts, WhatsApp alerts), Pro (₹149 / ₹299, 20 posts, PDF export, priority listing). Plan stored in `businesses.plan` (default `"starter"`). Post limits enforced server-side in `POST /api/profiles/:slug/posts` returning `error: "POST_LIMIT_REACHED"`. Pricing page at `/app/upgrade`. Plan config: `artifacts/sevu/src/lib/plans.ts` (UI) + `artifacts/api-server/src/lib/plans.ts` (limits including `maxCompletionsPerDay`: 5/15/30 for starter/growth/pro)
- **Jobs Done Counter (anti-spam)**: Each `service_log` has independent `completionStatus` (`pending` | `completed`) and `completedAt` timestamp — separate from `paymentStatus`. `PATCH /api/service-logs/:id/complete` enforces 3 anti-spam rules: (1) `DUPLICATE_TODAY` — same `customerId` already completed today; (2) `TOO_EARLY` — log < 30 min old; (3) `DAILY_CAP_REACHED` — plan's `maxCompletionsPerDay` reached. On success, increments `profiles.totalJobs` (busts cache). `PATCH /api/service-logs/:id/undo-complete` reverts within a 10-min window, decrements counter. `DELETE /api/service-logs/:id` decrements counter when deleting a completed log. `GET /api/service-logs/jobs-count` returns `{total, today, plan, dailyCap, remaining}`. Frontend `services.tsx` shows 4-button card footer `[Mark Complete | Mark Paid | View | Delete]` with live 1-Hz undo countdown timer (`Undo (M:SS)`) and inline error banners (Hinglish messages).
